...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Add event listener for AI智能解讀 button that will be added dynamically
    document.addEventListener('click', function(e) {
        if (e.target && (e.target.id === 'ai-embed-chat' || e.target.closest('#ai-embed-chat'))) {
            // 獲取當前卦象資訊
            const resultExists = document.querySelector('.divination-result');
            ...
            const primarySymbol = document.querySelector('.gua-symbol').textContent;
            const primaryVerse = document.querySelector('.gua-verse').textContent;
            const primaryMeaning = document.querySelector('.gua-meaning').textContent;
            
            // 準備變爻資訊
            let changingLinesInfo = "";
            const changingLinesTable = document.querySelector('.lines-table');
            if (changingLinesTable) {
                const movingLines = document.querySelectorAll('.lines-table .moving-line');
                changingLinesInfo = `變爻: ${movingLines.length}個，位置: `;
                movingLines.forEach(line => {
                    const cells = line.querySelectorAll('td');
                    if (cells.length >= 2) {
                        changingLinesInfo += `${cells[0].textContent}(${cells[1].textContent}) `;
                    }
                });
            }
            ...
            // 綁定關閉按鈕事件
            const closeBtn = aiChatModal.querySelector('.close-btn');
            closeBtn.addEventListener('click', () => {
            ...
            <div class="divination-result">
                ...
                <div class="ai-interpretation-actions" data-aos="fade-up" data-aos-delay="1200">
                    <button id="ai-explain-btn" class="btn btn-primary" onclick="document.getElementById('ai-button').click()">
                        <span class="btn-text">AI解析大師</span>
                        <span class="btn-glow"></span>
                    </button>
